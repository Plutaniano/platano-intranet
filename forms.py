...
class RegisterForm(FlaskForm):
    id = TextField('Código A (somente números)', validators=[DataRequired(), Length(min=5, max=5)])
...
class LoginForm(FlaskForm):
    id = TextField('Código A',                   validators=[DataRequired()])
    password = PasswordField('Senha',            validators=[DataRequired()])


class ForgotForm(FlaskForm):
    id = TextField('Email',                      validators=[DataRequired(), Length(min=6, max=40), Email()])
